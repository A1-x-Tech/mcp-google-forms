...
test("raw_request defaults to GET with the Bearer token", async () => {
  ...
  try {
    const res = await tools.raw_request({ path: "v1/forms/abc" });
    assert.equal(res.isError, undefined);
    assert.equal(calls[0].method, "GET");
    assert.equal(calls[0].url, "https://forms.googleapis.com/v1/forms/abc");
...
test("raw_request POSTs a JSON body to a relative path", async () => {
  ...
  try {
    await tools.raw_request({
      path: "v1/forms/abc:batchUpdate",
      method: "POST",
      body: { requests: [{ deleteItem: { location: { index: 0 } } }] },
    });
    assert.equal(calls[0].method, "POST");
    assert.equal(calls[0].url, "https://forms.googleapis.com/v1/forms/abc:batchUpdate");
    assert.deepEqual(calls[0].body, { requests: [{ deleteItem: { location: { index: 0 } } }] });
  } finally {
    restore();
  }
});
