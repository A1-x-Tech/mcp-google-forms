/**
 * The server talks to the Google Forms API v1 (https://forms.googleapis.com,
...
/**
 * Normalized question types accepted by add_question; the client maps them to
 * the API's wire Question union (textQuestion, choiceQuestion, scaleQuestion,
 * dateQuestion, timeQuestion, ratingQuestion). fileUploadQuestion is read-only
 * in the API and cannot be created; grids (questionGroupItem) go through
 * raw_request.
 */
...
/** Rating icon, normalized; mapped to STAR / HEART / THUMB_UP by the client. */
export type RatingIconType = "star" | "heart" | "thumb_up";

/** Email collection mode (API wire values, passed through). */
export type EmailCollectionType = "DO_NOT_COLLECT" | "VERIFIED" | "RESPONDER_INPUT";

/** Watch event types (API wire values, passed through). */
export type WatchEventType = "RESPONSES" | "SCHEMA";
...
export interface GoogleFormsConfig {
  ...
  accessToken?: string;
  /** API root. Defaults to https://forms.googleapis.com. */
  apiBase: string;
