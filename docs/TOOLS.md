...
| Tool | Description |
|---|---|
| `create_form` | Creates a form. The API accepts only `title` + `document_title` at creation — add questions and settings afterwards. **API-created forms start unpublished**; `publish: true` chains a `setPublishSettings` call so the form immediately accepts responses. Returns `formId`, `responderUri`, `revisionId`, `publishSettings`. |
...
| `update_form_info` | Changes `title` / `description` / `document_title`; the updateMask is computed from the provided fields (at least one required). |
| `update_form_settings` | Toggles quiz mode (`is_quiz`) and/or email collection (`DO_NOT_COLLECT` / `VERIFIED` / `RESPONDER_INPUT`). |
